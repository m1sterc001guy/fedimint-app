...
class BleTapController(
    ...
    companion object {
        ...
        /** Keep notifications comfortably inside one ATT PDU. */
    ...
    fun startSending(uuidString: String, blob: ByteArray) {
        ...
        val payload = BluetoothGattCharacteristic(
            CHAR_PAYLOAD_UUID,
            // READ as well as NOTIFY: the receiver reads this characteristic once
            // it has registered for notifications, and that read is what tells us
            // it is safe to start pushing. A read is used rather than a write
            // because client writes are the operation broken on Android 17.
        ...
        override fun onConnectionStateChange(device: BluetoothDevice, status: Int, newState: Int) {
            ...
            when (newState) {
                ...
                BluetoothProfile.STATE_DISCONNECTED -> {
                    ...
                    if (allSent) {
                        logd("central disconnected after full payload, delivered")
    ...
    private fun beginPush() {
        ...
        logd("beginPush attempt=$attempt blob=${blob.size}B mtu=$mtu dataSize=$dataSize frames=${sendQueue.size}")
        sendStatus("writing")
        pushNext()
    }
    ...
    private fun pushNext() {
        ...
        if (chunk == null) {
            allSent = true
            pushing = false
            logd("attempt $attempt fully pushed, awaiting receiver disconnect")
            sendStatus("sent")
            // The receiver confirms by disconnecting. If it doesn't, it wasn't
            // listening yet - send the whole thing again rather than waiting for a
            // readiness signal it may be incapable of sending.
    ...
    private fun notifyChunk(
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            val res = server.notifyCharacteristicChanged(device, ch, false, value)
            if (res != BluetoothStatusCodes.SUCCESS) logd("notifyCharacteristicChanged code=$res")
